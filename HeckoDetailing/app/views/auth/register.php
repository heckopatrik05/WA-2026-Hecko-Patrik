...
<div class="form-container" style="margin-top: 2rem; margin-bottom: 2rem;">
    ...
    <form action="<?= BASE_URL ?>/index.php?url=auth/register" method="post">
        <div class="form-grid">
...
<script>
    ...
    togglePasswordBtn.addEventListener('click', function () {
        // Zjistíme aktuální typ a přepneme ho
    ...
    passwordInput.addEventListener('input', function() {
        ...
        // Pokud je políčko prázdné, vyresetujeme vzhled
        if (!val) {
        ...
        if (/[^A-Za-z0-9]/.test(val)) score += 1;         // Obsahuje speciální znak (např. @, #, !)

        // Změna barvy a textu na základě nasbíraných bodů
        if (score <= 2) {
            strengthBar.style.width = '33%';
            strengthBar.style.backgroundColor = '#ef4444'; // Červená barva
            strengthText.textContent = 'Slabé heslo (přidejte čísla a velká písmena)';
            strengthText.style.color = '#ef4444';
        } else if (score === 3 || score === 4) {
            strengthBar.style.width = '66%';
            strengthBar.style.backgroundColor = '#f59e0b'; // Žlutá barva
            strengthText.textContent = 'Středně silné heslo (ideálně přidejte speciální znak)';
            strengthText.style.color = '#f59e0b';
        } else if (score === 5) {
            strengthBar.style.width = '100%';
            strengthBar.style.backgroundColor = '#10b981'; // Zelená barva
